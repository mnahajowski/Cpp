...
Table Table::operator+(const Table &otherTable) {
	...
	result.setNewSize(length + otherTable.length);
...
Table Table::operator+(int number) {
	Table result;

	result.setNewSize(length + 1);
	result.tab = new int[result.length];

	for (int i = 0; i < (result.length); i++) {
		result.vSetValueAt(i, tab[i]);

	}

	result.vSetValueAt(result.length - 1, number);

	return result;
}

/*
Table Table::operator-=(int liczba) {
	Table *result = new Table();

	result->setNewSize((this->length) - liczba);
...
Table Table::operator/=(int liczba) {
	...
	int index = 0;
	for (int i = 0; i < result->length; i++) {
		result->vSetValueAt(i, this->tab[index]);
		index += liczba;
	}
	return *result;
}*/
